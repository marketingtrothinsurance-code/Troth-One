...
import { franchiseeBusinessSummary, franchiseeBusinessTrend, franchiseeDashboardUpdates } from '../data/franchiseeData'
...
export function FranchiseeDashboard({apps,customers,leads,notifications,onNavigate,onToast}:Props){
  ...
  const todayPrefix=new Intl.DateTimeFormat('en-GB',{day:'2-digit',month:'short'}).format(today)
  const todayLabel=new Intl.DateTimeFormat('en-GB',{day:'2-digit',month:'short',year:'numeric'}).format(today)
  const dueFollowUps=leads.filter(lead=>lead.nextFollowUp.startsWith(todayPrefix))
  ...
  const trendMax=Math.max(...franchiseeBusinessTrend.map(item=>item.value))
  ...
  return <div className="page franchisee-dashboard-page">
    <PageHeader eyebrow="PARTNER WORKSPACE" title="Good morning, Neha" description="Focus on today’s customer actions, sales pipeline and applications." actions={<button className="date-chip"><CalendarClock/> {todayLabel}</button>}/>
    <CompactTicker label="Franchise Updates" items={franchiseeDashboardUpdates} onSelect={item=>onNavigate(item.target)}/>
    <section className="stats-grid franchisee-kpi-grid">
      <StatCard label="Customers" value={customers.length} meta={`${customers.filter(customer=>customer.relationshipStatus==='Active').length} active relationships`} icon={<Users/>} onClick={()=>onNavigate('customers')}/>
      <StatCard label="Open Leads" value={openLeads.length} meta={`${dueFollowUps.length} follow-ups due today`} icon={<UserPlus/>} tone="blue" onClick={()=>onNavigate('crm')}/>
      <StatCard label="Active Applications" value={openApps.length} meta="Across your customers" icon={<FileWarning/>} tone="violet" onClick={()=>onNavigate('applications')}/>
      <StatCard label="Conversion" value={`${conversion}%`} meta={`${converted} converted lead`} icon={<TrendingUp/>} tone="green" onClick={()=>{sessionStorage.setItem('troth-crm-stage','Converted');onNavigate('crm')}}/>
      <StatCard label="Recorded Revenue" value={formatINR(franchiseeBusinessSummary.recordedRevenue,true)} meta="Mock ledger • September" icon={<TrendingUp/>} tone="green" onClick={()=>onNavigate('business')}/>
      <StatCard label="Pending Actions" value={pendingApps.length} meta="Applications need attention" icon={<FileWarning/>} tone="amber" onClick={()=>onNavigate('applications','Action Required')}/>
    </section>
    ...
    <section className="franchisee-performance-grid">
      <div className="panel franchisee-business-panel">
        <div className="panel-head"><div><b>Business & revenue</b><small>Six-month recorded revenue trend</small></div><button onClick={()=>onNavigate('business')}>View report</button></div>
        <div className="franchisee-business-content">
          ...
          <div className="franchisee-trend" aria-label="Six-month recorded revenue trend">{franchiseeBusinessTrend.map(item=><div key={item.month}><i style={{height:`${item.value/trendMax*100}%`}} title={`${item.month}: ${formatINR(item.value)}`}/><span>{item.month}</span></div>)}</div>
        </div>
      ...
      <div className="panel franchisee-status-panel">
    ...
    <section className="franchisee-operations-grid">
      ...
      <div className="panel franchisee-followups-panel">
        <div className="panel-head"><div><b>Today’s follow-ups</b><small>Customers and leads to contact</small></div><button onClick={()=>onNavigate('crm')}>View all <ArrowRight/></button></div>
      ...
      <div className="panel franchisee-notifications-panel">
