...
export function StatCard({label,value,meta,tone='navy',onClick,icon,active}:{label:string,value:string|number,meta:string,tone?:string,onClick?:()=>void;icon?:React.ReactNode;active?:boolean}) {
  return <button className={`stat-card tone-${tone}${active?' active':''}`} onClick={onClick} aria-pressed={onClick?Boolean(active):undefined}>{icon&&<i className="stat-card-icon">{icon}</i>}<span>{label}</span><strong>{value}</strong><small>{meta}</small>{onClick && <ArrowRight size={17}/>}</button>
}
