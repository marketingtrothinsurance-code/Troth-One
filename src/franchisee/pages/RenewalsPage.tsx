...
export function RenewalsPage({customers,cases,onViewCustomer,onStartRenewal,onViewCase}:{customers:FranchiseeCustomer[];cases:FranchiseeCase[];onViewCustomer:(customerId:string)=>void;onStartRenewal:(renewal:FranchiseeRenewal)=>string;onViewCase:()=>void}){
  ...
  const filtered=useMemo(()=>renewals.filter(item=>`${item.customerName} ${item.policyNumber} ${item.product}`.toLowerCase().includes(search.toLowerCase())&&(product==='All Products'||item.product===product)&&(status==='All'||item.status===status)&&matchesDays(item.daysRemaining,days)),[renewals,search,product,status,days])
  const pageRows=filtered.slice((page-1)*8,page*8)
  const setFilter=<T,>(setter:(value:T)=>void,value:T)=>{setter(value);setPage(1)}
  const reset=()=>{setSearch('');setDays('All');setProduct('All Products');setStatus('All');setPage(1)}
  return <div className="franchisee-renewals">
    ...
    <section className="fr-renewal-kpis">
      <Metric label="Total Renewals Due" value={renewals.length} meta="Across permitted customers" icon={<RefreshCw/>}/>
      <Metric label="Due in Next 30 Days" value={renewals.filter(item=>item.daysRemaining>=0&&item.daysRemaining<=30).length} meta="Immediate follow-up required" icon={<AlertTriangle/>} tone="red"/>
      <Metric label="Due in 31–60 Days" value={renewals.filter(item=>item.daysRemaining>=31&&item.daysRemaining<=60).length} meta="Approaching renewal" icon={<Clock3/>} tone="orange"/>
      <Metric label="Due in 61–90 Days" value={renewals.filter(item=>item.daysRemaining>=61&&item.daysRemaining<=90).length} meta="Comfortably ahead" icon={<CalendarClock/>} tone="teal"/>
      <Metric label="Overdue" value={renewals.filter(item=>item.daysRemaining<0).length} meta="Needs urgent attention" icon={<AlertTriangle/>} tone="red"/>
    </section>
