...
export interface FranchiseeAgent {id:string;franchiseeId:string;subFranchiseeId?:string;agentCode:string;name:string;mobile:string;email:string;designation:string;assignedCustomerIds?:string[];assignments:AgentLineAssignment[];activity:AgentActivity[];createdAt:string;updatedAt:string}
export interface AgentDraft {agentCode:string;name:string;mobile:string;email:string;designation:string;businessLineIds:string[];assignedCustomerIds:string[];subFranchiseeId?:string}
...
function seed(products:FranchiseeProduct[]):FranchiseeAgent[]{const lines=buildAgentBusinessLines(products),first=assignments(['insurance'],lines),second=assignments(['loans','mf'],lines);return[
  {id:'AG-1',franchiseeId,agentCode:'AGT-1001',name:'Amit Shah',mobile:'+91 98250 77142',email:'amit.shah@example.in',designation:'Insurance Advisor',assignedCustomerIds:[],assignments:first.map(line=>complete(line,Object.fromEntries(line.requirements.map(item=>[item.id,item.verificationRequired?'Verified':'Completed'])))),activity:[{id:'AA-1',title:'Insurance eligibility activated',detail:'All mandatory Insurance requirements were completed and verified. Eligibility was calculated automatically.',time:'29 Aug 2026, 11:35 AM',actor:'System'},{id:'AA-2',title:'Registration verified',detail:'IRDAI / POSP registration verified.',time:'29 Aug 2026, 11:30 AM',actor:'HO Compliance'}],createdAt:'2026-08-18T10:00:00.000Z',updatedAt:'2026-08-29T11:35:00.000Z'},
  {id:'AG-2',franchiseeId,agentCode:'AGT-1002',name:'Kavya Mehta',mobile:'+91 98980 34118',email:'kavya.mehta@example.in',designation:'Business Associate',assignedCustomerIds:[],assignments:second.map(line=>complete(line,line.businessLineId==='loans'?{'loans-process':'Completed','loans-kyc':'Pending HO Verification','loans-authorization':'Pending HO Verification'}:{'mf-process':'Completed','mf-kyc':'Pending HO Verification','mf-nism':'Pending HO Verification'})),activity:[{id:'AA-3',title:'Requirements submitted',detail:'KYC and certification evidence sent for Head Office verification.',time:'05 Sep 2026, 04:20 PM',actor:'Franchise Principal'},{id:'AA-4',title:'Business lines assigned',detail:'Loans and Mutual Funds assigned.',time:'03 Sep 2026, 10:10 AM',actor:'Franchise Principal'}],createdAt:'2026-09-03T10:00:00.000Z',updatedAt:'2026-09-05T16:20:00.000Z'}
]}

const storageKey='troth-franchisee-agents-v1'
const read=(products:FranchiseeProduct[])=>{try{const value=localStorage.getItem(storageKey);const rows=value?JSON.parse(value) as FranchiseeAgent[]:seed(products);return rows.map(agent=>({...agent,assignedCustomerIds:agent.assignedCustomerIds||[]}))}catch{return seed(products)}}
...
export const franchiseeAgentRepository={
  ...
  create:(draft:AgentDraft,products:FranchiseeProduct[],ownerFranchiseeId=franchiseeId)=>{const agent:FranchiseeAgent={id:crypto.randomUUID(),franchiseeId:ownerFranchiseeId,subFranchiseeId:draft.subFranchiseeId,agentCode:draft.agentCode,name:draft.name,mobile:draft.mobile,email:draft.email,designation:draft.designation,assignedCustomerIds:draft.assignedCustomerIds,assignments:assignments(draft.businessLineIds,buildAgentBusinessLines(products)),activity:[event('Agent onboarding started',`${draft.businessLineIds.length} business line(s) assigned. Eligibility remains gated by requirements.`)],createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};write([agent,...read(products)]);return agent},
